...
$query  = mysqli_query($db, "SELECT penjualan.kd_penjualan,penjualan.tgl_jual,penjualan.tgl_kirim,pelanggan.nm_plg,det_penjualan.kd_menu,det_penjualan.jumlah,det_penjualan.harga,menu.nama_menu,penjualan.tarif,SUM(det_penjualan.jumlah*det_penjualan.harga)AS totalbeli,SUM(det_penjualan.jumlah*det_penjualan.harga)+penjualan.tarif AS sub FROM penjualan JOIN det_penjualan ON det_penjualan.kd_penjualan=penjualan.kd_penjualan JOIN menu ON det_penjualan.kd_menu=menu.kd_menu JOIN pelanggan ON penjualan.id_pelanggan=pelanggan.id_pelanggan JOIN pembayaran ON pembayaran.kd_penjualan=penjualan.kd_penjualan WHERE DATE(penjualan.tgl_jual) BETWEEN '$tglmin' AND '$tglmax' AND penjualan.tipe_jual='Catering' AND pembayaran.status_bayar='Lunas'  GROUP BY det_penjualan.kd_penjualan ORDER BY penjualan.kd_penjualan ASC");


$query1 = mysqli_query($db, "SELECT SUM(penjualan.tarif+det_penjualan.harga*det_penjualan.jumlah) AS total FROM det_penjualan JOIN penjualan ON det_penjualan.kd_penjualan=penjualan.kd_penjualan JOIN pembayaran ON pembayaran.kd_penjualan=penjualan.kd_penjualan WHERE DATE(penjualan.tgl_jual) BETWEEN '$tglmin' AND '$tglmax' AND penjualan.tipe_jual='Catering' AND pembayaran.status_bayar='Lunas'");

$pdf->SetFont('Arial', 'B', 12);
$pdf->Cell(0, 5, 'Laporan Catering', '0', '1', 'C', false);
